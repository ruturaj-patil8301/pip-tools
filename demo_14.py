...
def get_package_info(data, package_name):
    for package_entry in data:
        ...
        if pkg.get("key").lower() == package_name.lower():
            # print(f"Found package: {pkg.get('key')}")
            # print(f"Package entry: {package_entry}")
...
def main(package_name):
    ...
    for dep in dependencies:
        ...
        required_version_spec = dep.get("required_version", "")
        #print(f"Dependency: {dep_name}, Installed: {installed_version}, Required: {required_version_spec}")
