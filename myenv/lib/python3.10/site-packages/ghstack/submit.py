...
from ghstack.types import GhNumber, GitCommitHash, GitHubNumber, GitHubRepositoryId, override_str_non_defaults
...
RE_STACK = re.compile(r"## PR Stack:\r?\n.*:\r?\n(\* [^\r\n]+\r?\n)+")
...
class Submitter:
    ...
    # Template to use for the commit message
    template: str = ""
    ...
    def run(self) -> List[DiffMeta]:
        ...
        logging.debug("Commits to submit and boundary: %s", commits_to_submit_and_boundary)

        commits_to_submit = [
            d for d in commits_to_submit_and_boundary if not d.boundary
        ]

        logging.debug("Commits to submit:\n")
        for commit in commits_to_submit:
        ...
        logging.debug("Commits to rebase and boundary: %s", commits_to_rebase_and_boundary)

        commits_to_rebase = [
            d for d in commits_to_rebase_and_boundary if not d.boundary
        ]
        logging.debug("Commits to rebase: %s", commits_to_rebase)
        ...
        logging.debug("Commit count: %s", commit_count)
        ...
        logging.debug("Commit index: %s", commit_index)
        ...
        logging.debug("diff_meta_index = %s", diff_meta_index)
        diffs_to_submit = [
            diff_meta_index[h.commit_id]
            for h in commits_to_submit
            if h.commit_id in diff_meta_index
        ]
        logging.debug("len diffs_to_submit = %s", len(diffs_to_submit))
        logging.debug("Diffs to submit: %s", diffs_to_submit)
    ...
    def prepare_updates(
        ...
        logging.debug("Starting prepare_updates")
        ...
        for commit in reversed(commits_to_rebase):
            ...
            logging.debug("parent_commit: %s", parent_commit)
            parent_diff_meta = diff_meta_index.get(parent)
            logging.debug("parent_diff_meta: %s", parent_diff_meta)
            ...
            logging.debug("diff_meta: %s", diff_meta)
            ...
            if parent in rebase_index or diff_meta is not None:
                ...
                logging.debug("new_orig: %s", new_orig)
    ...
    def elaborate_diff(
        ...
        """
        logging.debug("Starting elaborate_diff")
...
""".format(
        ...
        if self.existing_head == r["headRefName"]:
            username = r["author"]["login"]
            gh_number = r["headRefName"]
        else:
            username = m.group(1)
            gh_number = GhNumber(m.group(2))

        logging.debug("Username: %s", username)
        logging.debug("gh_number: %s", gh_number)
        ...
        logging.debug("pr_body: %s", pr_body)
        ...
        try:
            # if we're dealing with the first PR in the stack, use the existing head for
            # the rev-list command; otherwise use the orig branch
            branch = self.existing_head if self.existing_head else branch_orig(username, gh_number)
        ...
        logging.debug("remote_source_id: %s", remote_source_id)
    ...
    def process_commit(
        ...
        logging.debug("Starting process_commit")
        ...
        # Create bool for if we are updating a PR not part of the stack
        new_existing_pr = False
        if elab_diff is not None:
            if ghnum == self.existing_head and self.stack_header not in elab_diff.body:
                new_existing_pr = True

        logging.debug("new_existing_pr = %s", new_existing_pr)
        ...
        logging.debug("Push branches: %s", push_branches)
        logging.debug("Base branch: %s", base_branch)
        ...
        if elab_diff is None:
            ...
            if new_existing_pr:
                # Update the existing PR with the new stack information
                logging.debug("Updating PR %s with new stack information", elab_diff.number)
                _, default_body = self._default_title_and_body(diff, elab_diff.body)
                logging.debug("Default body: %s", default_body)
                self.github.patch(
                    f"repos/{self.repo_owner}/{self.repo_name}/pulls/{elab_diff.number}",
                    body=f"{elab_diff.body}\n\n{default_body}",
                )
                # update elaborated diff with new body
                elab_diff.body = f"{elab_diff.body}\n\n{default_body}"
                logging.debug("Updated body: %s", elab_diff.body)
            if not push_branches:
                ...
                if new_existing_pr:
                    # This is our first time updating a PR to be part of the
                    # stack, push the `next` branch to the remote
                    self._git_push(
                        [
                            push_spec(push_branches.next.commit.commit_id, branch_next(username, ghnum)),
                        ]
                    )
                else:
                    # Push the upstream PR updates to factor in the rebase commits.
                    # If a developer adds a new commit and rebases, ghstack correctly
                    # calculates the commits in the stack and the boundary, but we first
                    # create the PR for the new commit before updating the previous branches
                    # in the stack. The new PR includes the rebase commits in its
                    # history, meaning that the PR will have a lot of un-related commits.
                    # Even when the older PRs/branches get updated in push_updates, the
                    # new PR in the stack will still have the commits from the rebase
                    # since we don't update the head branch again after creating the PR.
                    # As such, we perform a push here to ensure that the existing PR
                    # branches get updated with the rebase.
                    if self.existing_head and self.existing_head == ghnum:
                        # For converted PRs, we can't use the ghnum convention, instead
                        # we directly use the name of the branch and the stored head
                        # commit hash in push_branches.
                        self._git_push([push_spec(push_branches.head.commit.commit_id, self.existing_head)])
                    else:
                        self._git_push(
                            [
                                push_spec(p[0], branch(username, ghnum, p[1]))
                                for p in push_branches
                            ]
                        )
                what = "Updated"
            new_pr = False
        ...
        if not new_pr and not new_existing_pr:
            ...
            logging.debug("Inserting metadata for the first time")
            stripped_summary = strip_mentions(diff.summary.rstrip())
            if new_existing_pr:
                # Insert ghstack-source-id before Pull Request resolved line
                constructed_commit_message = []
                stripped_summary = stripped_summary.split("\n")
                for line in stripped_summary:
                    if line.startswith("Pull Request resolved:") and self.direct:
                        constructed_commit_message.append(f"ghstack-source-id: {diff.source_id}")
                    constructed_commit_message.append(line)
                commit_msg = "\n".join(constructed_commit_message)
            else:
                commit_msg = "".join(
                    [
...
"""
    ...
    def _update_source_id(self, summary: str, elab_diff: DiffWithGitHubMetadata) -> str:
        logging.debug("Starting _update_source_id")
        logging.debug("Summary: %s", summary)
        m_local_source_id = RE_GHSTACK_SOURCE_ID.search(summary)
        if m_local_source_id is None and not self.existing_head:
            ...
            if elab_diff.remote_source_id is None:
                if elab_diff.head_ref != self.existing_head:
            ...
            elif local_source_id != elab_diff.remote_source_id and not self.force and elab_diff.head_ref != self.existing_head:
                logging.debug(
                    "elab_diff.remote_source_id = %s", elab_diff.remote_source_id
                )
    ...
    def _resolve_gh_branch(
        ...
        logging.debug("Starting _resolve_gh_branch for %s, %s, %s", kind, username, ghnum)
        if ghnum == self.existing_head and kind == "head":
            branch_name = self.existing_head
        else:
            branch_name = branch(username, ghnum, kind)
        logging.debug("Branch name: %s", branch_name)
        remote_ref = self.remote_name + "/" + branch_name
    ...
    def _resolve_gh_branches(self, username: str, ghnum: GhNumber) -> GhBranches:
        logging.debug("Starting _resolve_gh_branches")
        push_branches = GhBranches()
    ...
    def _create_non_orig_branches(
        ...
        logging.debug("Starting _create_non_orig_branches")
        ...
        if elab_diff is not None:
            if self.existing_head and self.stack_header not in elab_diff.body:
                # We're working with a PR that previously was not part of a stack.
                # We treat the existing PR branch as the head, but we still need
                # to create the orig and next branches
                push_branches = GhBranches()
                existing_head = GitCommitHash(
                    self.sh.git(
                        "rev-parse", f"{self.remote_name}/{self.existing_head}"
                    )
                )

                push_branches.head.update(GhCommit(existing_head, diff.tree))
            else:
        ...
        if not self.direct:
            ...
            if base_diff_meta is not None:
                logging.debug("base_diff_meta: %s", base_diff_meta)
                ...
                if base_diff_meta.next == base_diff_meta.head:
                    if self.existing_head and base_diff_meta.ghnum == self.existing_head:
                        base_branch = GitCommitHash(self.existing_head)
                    else:
                        # use head
            ...
            logging.debug("new_base: %s", new_base)
            logging.debug("base_branch: %s", base_branch)
        ...
        if (
            ...
            logging.debug("_create_non_orig_branches new_head: %s", new_head)
            if self.direct:
                ...
                push_branches.head.update(GhCommit(new_head, diff.tree))
        logging.debug("_create_non_orig_branches push_branches: %s", push_branches)
    ...
    def _create_pull_request(
        ...
        logging.debug("Starting _create_pull_request")
        title, default_body = self._default_title_and_body(diff, None)
        head_ref = branch_head(self.username, ghnum)

        if self.direct:
            if base_diff_meta is None:
                base_ref = self.base
            elif self.existing_head and base_diff_meta.ghnum == self.existing_head:
                base_ref = self.existing_head
            else:
        ...
        template_with_commit_message = f"{diff.title}\n\n{self.template}"
        user_input_body = self.sh.get_user_input(template_with_commit_message)
        body = f"{user_input_body}\n{default_body}"
        logging.info(f"PR body for <{diff.title}>:\n\n{body}")
    ...
    def push_updates(
        self, diffs_to_submit: List[DiffMeta]
    ) -> None:
        logging.debug("Starting push_updates")
        ...
        for s in reversed(diffs_to_submit):
            ...
            if self.direct:
                ...
                if self.existing_head and self.existing_head in base_kwargs["base"]:
                    base_kwargs["base"] = self.existing_head
            else:
            ...
            self.github.patch(
                ...
                # NB: this substitution will take effect on direct and non-direct PRs
                body=RE_STACK.sub(
                    stack_desc,
                    s.body,
                ),
            ...
            # branch
            logging.debug("Pushing %s", s.push_branches)
            ...
            for diff, b in s.push_branches:
                ...
                if s.ghnum == self.existing_head and b == "head":
                    q_branch = GitCommitHash(self.existing_head)
                else:
                    q_branch = branch(s.username, s.ghnum, b)
                q.append(push_spec(diff, q_branch))
            logging.debug("q: %s", q)
        # Careful!  Don't push master.
        # TODO: These pushes need to be atomic (somehow)
        logging.debug("base_push_branches: %s", base_push_branches)
        logging.debug("push_branches: %s", push_branches)
        logging.debug("force_push_branches: %s", force_push_branches)
    ...
    def _git_push(self, branches: Sequence[str], force: bool = False) -> None:
        ...
        try:
            logging.debug("Pushing %s", branches)
            self.sh.git(
                "push",
                self.remote_name,
                *(["--force"] if force else []),
