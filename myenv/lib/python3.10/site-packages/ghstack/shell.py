...
import sys
import tempfile
from typing import Any, Dict, IO, Optional, overload, Sequence, Tuple, TypeVar, Union, List
...
class Shell(object):
    ...
    def get_git_config_user_editor(self) -> List[str]:
        """
        Get the user's editor from git config.
        Defaults to vim if not set in either git config or environment.
        """
        try:
            editor = self.git("config", "--get", "core.editor")
            if not editor:
                editor = os.environ.get("EDITOR", "vim")
        except Exception:
            editor = os.environ.get("EDITOR", "vim")

        # Users might have set the editor to something like "vscode --wait"
        # which will cause subprocess.run to fail. We need to split the
        # command into a list of arguments.
        editor = shlex.split(editor)

        return editor

    def get_user_input(self, template: str = None):
        """
        Create a tempfile populated with the provided template and return the contents.
        """
        ...
        with tempfile.NamedTemporaryFile(suffix=".tmp", delete=False, mode="w+t") as tf:
            ...
            editor = self.get_git_config_user_editor()
            edit_command = editor + [file_path]
            logging.info(f"Running command: {edit_command}")
            process = subprocess.run(edit_command)

            if process.returncode != 0:
                raise RuntimeError("Editor exited with non-zero status")
