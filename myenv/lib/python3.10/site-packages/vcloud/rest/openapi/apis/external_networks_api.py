...
class ExternalNetworksApi(object):
    ...
    def get_all_external_networks(self, page, page_size, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_all_external_networks(page, page_size, callback=callback_function)
        ...
        if kwargs.get('callback'):
            return self.get_all_external_networks_with_http_info(page, page_size, **kwargs)
        else:
            (data) = self.get_all_external_networks_with_http_info(page, page_size, **kwargs)
            return data

    def get_all_external_networks_with_http_info(self, page, page_size, **kwargs):
        """
        Get all external networks.
        Get all external networks. Results can be filtered by id, name, backing type (networkBackings.values.backingType) and context (_context). Supported contexts are: Provider vDC ID (_context==providerVdcId) - | Returns all the external networks which are available to a specific Provider vDC. Org vDC ID (_context==orgVdcId) - | Returns all the external networks which are available to a specific Org vDC. vCenter ID And Resource Pool Moref (_context==vCenterId;_context==rpMoref) - | Returns all the external networks accessible to a given vCenter resource pool. Org vDC ID And Edge Deployment Mode (_context==orgVdcId;_context==anEdgeDeploymentMode) - | Returns all the external networks to which an edge gateway can connect. Edge Deployment Mode can be 'standaloneEdgeDeployment' or 'haEdgeDeployment'. Deployment mode specifies whether to use both primary edge cluster and secondary edge cluster or just primary edge cluster to determine external network accessibility. Edge clusters are determined via vDC Network Profile for input Org vDC. Org vDC ID And Dedicatable External Networks (_context==orgVdcId;_context==dedicatable) - | Dedicatable only shows external networks that have no connected Edge Gateways. 
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_all_external_networks_with_http_info(page, page_size, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    ...
                    " to method get_all_external_networks" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'page' is set
        if ('page' not in params) or (params['page'] is None):
            raise ValueError("Missing the required parameter `page` when calling `get_all_external_networks`")
        # verify the required parameter 'page_size' is set
        if ('page_size' not in params) or (params['page_size'] is None):
            raise ValueError("Missing the required parameter `page_size` when calling `get_all_external_networks`")

        if 'page' in params and params['page'] < 1:
            raise ValueError("Invalid value for parameter `page` when calling `get_all_external_networks`, must be a value greater than or equal to `1`")
        if 'page_size' in params and params['page_size'] > 128:
            raise ValueError("Invalid value for parameter `page_size` when calling `get_all_external_networks`, must be a value less than or equal to `128`")
        if 'page_size' in params and params['page_size'] < 0:
            raise ValueError("Invalid value for parameter `page_size` when calling `get_all_external_networks`, must be a value greater than or equal to `0`")
        ...
        return self.api_client.call_api('/1.0.0/externalNetworks', 'GET',
                                        path_params,
                                        query_params,
                                        header_params,
                                        body=body_params,
                                        ...
                                        response_type='ExternalNetworks',
