...
class EdgeClustersApi(object):
    ...
    def create_edge_cluster(self, edge_cluster, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.create_edge_cluster(edge_cluster, callback=callback_function)
        ...
        if kwargs.get('callback'):
            return self.create_edge_cluster_with_http_info(edge_cluster, **kwargs)
        else:
            (data) = self.create_edge_cluster_with_http_info(edge_cluster, **kwargs)
            return data

    def create_edge_cluster_with_http_info(self, edge_cluster, **kwargs):
        """
        Create a new Edge Cluster
        Create a new Edge Cluster. An Edge Cluster is defined by a Resouce Pool and Storage Profile for deploying Edge Gateways. It can subsequently be assigned to a given vDC Network Profile as a Primary or Secondary Edge Cluster. 
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.create_edge_cluster_with_http_info(edge_cluster, callback=callback_function)
        ...
        if ('edge_cluster' not in params) or (params['edge_cluster'] is None):
            raise ValueError("Missing the required parameter `edge_cluster` when calling `create_edge_cluster`")
        ...
        return self.api_client.call_api('/1.0.0/edgeClusters', 'POST',
                                        path_params,
                                        query_params,
                                        header_params,
                                        body=body_params,
    ...
    def get_edge_clusters(self, page, page_size, **kwargs):
        """
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_edge_clusters(page, page_size, callback=callback_function)
        ...
        kwargs['_return_http_data_only'] = True
        if kwargs.get('callback'):
            return self.get_edge_clusters_with_http_info(page, page_size, **kwargs)
        else:
            (data) = self.get_edge_clusters_with_http_info(page, page_size, **kwargs)
            return data

    def get_edge_clusters_with_http_info(self, page, page_size, **kwargs):
        """
        Get all Edge Clusters in the system
        Retrieves all Edge Clusters. Results can be filtered by id, name, vCenter (resourcePool.vcId), externalNetworkId and orgVdcId. <ul> <li>externalNetworkId - |   The URN of external Network.   Filters all edgeClusters that are accessible to externalNetworkId.   externalNetworkId filter is supported from version 35.2   Example: (externalNetworkId==urn:vcloud:network:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx) </li> <li>orgVdcId - |   The URN of Org vDC.   Filters all edgeClusters that are available to an Org vDC.   orgVdcId filter is supported from version 36.0   Example: (orgVdcId==urn:vcloud:vdc:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx) </li> </ul> Error will be thrown if both externalNetworkId and orgVdcId filters are supplied. 
        This method makes a synchronous HTTP request by default. To make an
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.get_edge_clusters_with_http_info(page, page_size, callback=callback_function)
        ...
        for key, val in iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method get_edge_clusters" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'page' is set
        if ('page' not in params) or (params['page'] is None):
            raise ValueError("Missing the required parameter `page` when calling `get_edge_clusters`")
        # verify the required parameter 'page_size' is set
        if ('page_size' not in params) or (params['page_size'] is None):
            raise ValueError("Missing the required parameter `page_size` when calling `get_edge_clusters`")

        if 'page' in params and params['page'] < 1:
            raise ValueError("Invalid value for parameter `page` when calling `get_edge_clusters`, must be a value greater than or equal to `1`")
        if 'page_size' in params and params['page_size'] > 128:
            raise ValueError("Invalid value for parameter `page_size` when calling `get_edge_clusters`, must be a value less than or equal to `128`")
        if 'page_size' in params and params['page_size'] < 0:
            raise ValueError("Invalid value for parameter `page_size` when calling `get_edge_clusters`, must be a value greater than or equal to `0`")
        ...
        return self.api_client.call_api('/1.0.0/edgeClusters', 'GET',
                                        path_params,
                                        query_params,
                                        header_params,
                                        body=body_params,
