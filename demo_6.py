...
def parse_reverse_package_data(data):
    ...
    lines = data.strip().split('\n')
    for line in lines:
        # Extract the main package version (e.g., `cryptography==42.0.8`)
        ...
        match = re.match(r'^[├└│─]*\s*(\S+)\s\[requires:\s*(.*)\]', line)
        if match:
            name = match.group(1).split('==')[0]  # Get package name only for reverse case
            required_version = match.group(2).strip()
            if required_version.startswith('cryptography'):
                required_version = required_version[len('cryptography'):].strip()
            packages.append((name, required_version))

    return installed_version, packages


def validate_reverse_dependencies(installed_version, packages):
    problems_found = False

    for name, required_version in packages:
        if required_version == '':
            print(f"{name}: requires cryptography [OK]")
            continue

        try:
            specifier_set = specifiers.SpecifierSet(required_version)
            if not version.parse(installed_version) in specifier_set:
                print(f"{name}: requires cryptography {required_version} [NOT OK]")
                problems_found = True
            else:
                print(f"{name}: requires cryptography {required_version} [OK]")
...
def main():
    ...
    if package_data:
        if args.reverse:
            ...
            if installed_version and validate_reverse_dependencies(installed_version, package_list):
